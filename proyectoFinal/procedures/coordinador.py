...
sys.path.append("./../")
from proyectoFinal.libs.sqlBd import Bd
...
def calculaFrecuencias(listaNumeros):
	frecuencias = {}
	for a in range(1,31):
		frecuencias[ a ] = 0
...
def guardaEnBd(ip_origen, numeroServer, frecuencias, relojObject, nombreEquipo, dbName=None):
	resultado = {'ok':True, 'description':frecuencias}
	...
	a = bd.doQuery("""
	INSERT INTO envios(id, date_added, ip_origen, nombre_equipo, num_jugador) 
	VALUES('{}', '{}', '{}', '{}', '{}');""".format(
			id_next, str(datetimeServer), str(ip_origen), str(nombreEquipo), str(numeroServer)
		)
	)

	queriesList = ["START TRANSACTION"]

	for numero in frecuencias:
		query = "INSERT INTO frecuencias(id_envio, numero, frecuencia) VALUES('{}', '{}', '{}');".format(
			id_next, numero, frecuencias[numero]
		)
		queriesList.append( query )

	queriesList.append("COMMIT;")

	res = bd.doTransaction(queriesList)
	if res == "exito":
		resultado['ok'] = True
		resultado['description'] = "Resultado almacenado"
	else:
		resultado['ok'] = False
		resultado['description'] = "Resultado NO guardado"

	return resultado

def send_to_others_servers(ip_origen, numeroServer, frecuencias, nombreEquipoOrigen, servidor_destino):
	my_address = get_ip()
...
def sendResultToOtherServer(ip_origen, numeroServer, frecuencias, nombreEquipoOrigen, equipos_a_enviar):
	...
	for equipo in equipos_a_enviar:
		if equipo['direccion'] == my_address:
			continue
		send = threading.Thread(target=send_to_others_servers, name="Envia a servidores", args=(ip_origen, numeroServer, frecuencias, nombreEquipoOrigen, equipo,))
		send.start()
		send.join()
